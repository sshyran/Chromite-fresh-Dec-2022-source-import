...
def test_execute(monkeypatch, tmp_path):
  ...
  name_metadata = spiderlib.TestEbuild(
      7, 'Description of category/name-1', 'http://home.com', 'Google', '0/0',
      'name-1.tar.gz', 'mirror', 'category/name', 'category/name yrogetac/foo',
      'yrogetac/foo', 'use ? (category/name)', 'use +flag -foo ',
      'ssalce\trandomstuff\teclass ')
  eman_metadata = spiderlib.TestEbuild(8, bdepend='category/name')
  (_package_files, oak_ebuilds,
   oak_ebuilds_metadata) = spider_testables.create_ebuilds(
       tmp_path,
       test_oak,
       {
