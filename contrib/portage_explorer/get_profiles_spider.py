...
def execute(output: spiderlib.SpiderOutput):
  ...
  for overlay in output.overlays:
    overlay_profiles_path = (Path(constants.SOURCE_ROOT) / overlay.path /
                             'profiles')
    for profile_path in overlay_profiles_path.glob('*/'):
      if profile_path.is_dir():
        ...
        profile = spiderlib.Profile(profile_id, profile_src_path,
                                    profile_name)
        ...
        elif board_name not in board_profiles and profile_name == 'base':
          board_profiles[board_name] = profile
      overlay.profiles = sorted(overlay.profiles,
