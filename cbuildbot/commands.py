...
_AUTOTEST_RPC_CLIENT = (
    "/b/build_internal/scripts/slave-internal/autotest_rpc/"
...
@failures_lib.SetFailureType(
    failures_lib.SuiteTimedOut, timeout_util.TimeoutError
)
def RunHWTestSuite(
    build,
    suite,
    ...
    try:
        ...
        swarming_args = _CreateSwarmingArgs(
            build, suite, board, priority, timeout_mins
        )
        ...
        if not result.HasValidSummary():
            ...
            logging.error(
                "No valid task summary json generated, output:%s", result.stdout
            )
            ...
            if result.returncode in lab_warning_codes:
                ...
                result.returncode in infra_error_codes
                or result.returncode in proxy_failure_codes
            ):
...
def _CreateSwarmingArgs(
    build, suite, board, priority, timeout_mins=None, run_skylab=False
):
...
def _HWTestCreate(cmd, debug=False, **kwargs):
    ...
    if debug:
        logging.info(
            "RunHWTestSuite would run: %s", cros_build_lib.CmdToStr(start_cmd)
        )
    else:
...
def _HWTestWait(cmd, job_id, **kwargs):
    ...
    try:
        ...
        if (
            ...
            or result.GetValue("internal_failure")
        ):
            raise
        else:
            logging.error(
                "wait_cmd has lab failures: %s.\nException will be raised "
                "in the next json_dump run.",
                e.msg,
            )
    for output in result.GetValue("outputs", ""):
        sys.stdout.write(output)
    ...
    for curr_dir, _subdirs, files in os.walk(test_results_dir):
        for curr_file in files:
            ...
            if curr_file.endswith(".dmp"):
                ...
                cros_build_lib.run(
                    ["minidump_stackwalk", minidump, symbol_dir],
                    cwd=cwd,
                    enter_chroot=True,
                    check=False,
                    stderr=True,
                ...
                with open(full_file_path) as f:
                    for line in f:
                        ...
                        if stackline_match:
                            ...
                            line = (
                                line[:frame_end] + board_path + line[frame_end:]
                            )
                        log_content += line
...
def ConvertResultsProtoToJson(results, board_name):
    ...
    container_images = container_metadata.containers[board_name].images
