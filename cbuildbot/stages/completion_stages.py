...
class PublishUprevChangesStage(generic_stages.BuilderStage):
    ...
    def CheckMasterBinhostTest(self, buildbucket_id):
        ...
        if (
            ...
            if not stages:
                logging.warning(
                    "no %s stage found in build %s", stage_name, buildbucket_id
                )
                return False
        ...
        logging.warning(
            "Not valid build_stage_id %s or no %s found",
    ...
    def CheckSlaveUploadPrebuiltsTest(self):
        ...
        if not self._run.config.master:
            ...
            logging.info(
                "In debug environment, return CheckSlaveUploadPrebuiltsTest"
                "as True"
            )
            return True
        elif (
            ...
            if passed_set.issuperset(important_set):
                ...
                logging.warning(
                    "slave %s didn't pass %s", remaining_set, stage_name
                )
                return False
        else:
