...
class Builder(object):
    ...
    def _SetReleaseTag(self):
        ...
        logging.debug(
            "Saved release_tag value for run: %r", self._run.attrs.release_tag
        )
    ...
    def _ReExecuteInBuildroot(self, sync_instance):
        ...
        if self._run.options.buildbot and hasattr(
        ...
        if pool:
            filename = os.path.join(
                self._run.options.buildroot, "validation_pool.dump"
            )
            pool.Save(filename)
    ...
    def _GetBootstrapStage(self):
        ...
        if (
            patches_needed
            or self._run.options.test_bootstrap
            or chromite_branch != cbuildbot_branch
        ):
            buildstore = BuildStore()
            stage = sync_stages.BootstrapStage(
                self._run, buildstore, self.patch_pool
            )
        return stage
