    ...
    def Install(self, cpv, url):
        ...
        with osutils.TempDir(sudo_rm=True) as tempdir:
            ...
            with open(
                ...
                link.mergeme(
                    tempdir, self._sysroot, content_file, None, "", {}, None
                )
...
def ShouldGetSymbols(cpv, vardb, remote_symbols):
    ...
    return (
        "separatedebug" in features
        and not "/usr/lib/debug/" in contents
...
def ListBinhost(binhost, binhost_cache=None):
    ...
    for p in pkgindex.packages:
        if p.get("DEBUG_SYMBOLS") == "yes":
            ...
            symbols[p["CPV"]] = os.path.join(
                base_url, path.replace(".tbz2", DEBUG_SYMS_EXT)
            )
...
def _InstallOne(sysroot, debug, args):
    ...
    with DebugSymbolsInstaller(
        vartree, gs_context, sysroot, not debug
    ) as installer:
