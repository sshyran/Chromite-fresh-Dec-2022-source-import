...
class CreateManifestSnapshotTest(
    cros_test_lib.MockTempDirTestCase, repo_manifest_unittest.XMLTestCase
):
    ...
    def testMakeUniqueRefMain(self):
        ...
        ref1 = create_manifest_snapshot._MakeUniqueRef(
            self.project_c, "base", used
        )
        ref2 = create_manifest_snapshot._MakeUniqueRef(
            self.project_c, "base", used
        )
        ref3 = create_manifest_snapshot._MakeUniqueRef(
            self.project_c, "base", used
        )
    ...
    def testMainSnapshots(self):
        ...
        self.mock_git_push.assert_has_calls(
            [
                mock.call(
                    os.path.join(self.repo_root, "project/a"),
                    "f01dab1e",
                    git.RemoteRef("origin", "refs/snap"),
                    dry_run=False,
                ),
                mock.call(
                    os.path.join(self.repo_root, "project/b"),
                    "cafe1234",
                    git.RemoteRef("origin", "refs/snap"),
                    dry_run=False,
                ),
                mock.call(
                    os.path.join(self.repo_root, "project/c"),
                    "deadbeef",
                    git.RemoteRef("origin", "refs/snap"),
                    dry_run=False,
                ),
                mock.call(
                    os.path.join(self.repo_root, "dupe"),
                    "d1",
