...
def _GetSpecificVersionUrl(git_url, revision, time_to_wait=600):
    ...
    def _wait_msg(_remaining):
        logging.info(
            "Repository does not yet have revision %s.  Sleeping...", revision
        )
...
class ChromeEBuild(portage_util.EBuild):
    ...
    chrome_version_re = re.compile(r".*-(%s|9999).*" % (_CHROME_VERSION_REGEX))
    chrome_version = ""
...
def GetChromeRevisionListLink(old_chrome, new_chrome, chrome_rev):
    ...
    return GetChromeRevisionLinkFromVersions(
        old_chrome.chrome_version, new_chrome.chrome_version
    )
...
def MarkChromeEBuildAsStable(
    ...
    if stable_candidate and chrome_rev in _REV_TYPES_FOR_LINKS:
        cbuildbot_alerts.PrintBuildbotLink(
            "Chromium revisions",
            GetChromeRevisionListLink(stable_candidate, new_ebuild, chrome_rev),
        )
...
def main(argv):
    ...
    if options.force_version and chrome_rev not in (
        ...
        parser.error(
            "--force_version is not compatible with the %r "
            "option." % (chrome_rev,)
        )
    ...
    if chrome_rev == constants.CHROME_REV_LOCAL:
        ...
        if "." in options.force_version:
            ...
            version_to_uprev = _GetSpecificVersionUrl(
                options.chrome_url, commit_to_use
            )
    ...
    stable_candidate = FindChromeUprevCandidate(
        stable_ebuilds, chrome_rev, sticky_branch
    )
    ...
    work_branch = cros_mark_as_stable.GitBranch(
        constants.STABLE_EBUILD_BRANCH, tracking_branch, chrome_package_dir
    )
    ...
    if chrome_version_atom:
        ...
        for other_ebuild in constants.OTHER_CHROME_PACKAGES:
            ...
            (
                ...
                other_stable_ebuilds,
            ) = FindChromeCandidates(other_package_dir)
