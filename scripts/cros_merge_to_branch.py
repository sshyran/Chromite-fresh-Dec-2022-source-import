...
def _UploadChangeToBranch(work_dir, patch, branch, draft, dryrun):
    ...
    for reviewers in (reviewers, ()):
        try:
            ...
            if e.returncode == 128 and re.search(
                r'fatal: user ".*?" not found', e.stderr
            ):
                logging.warning(
                    "Some reviewers were not found (%s); "
                    "dropping them & retrying upload",
                    " ".join(reviewers),
                )
                continue
            raise
...
def _SetupWorkDirectoryForPatch(work_dir, patch, branch, manifest, email):
    ...
    logging.notice(
        "Setting up dir %s for uploading changes to %s",
        work_dir,
        patch.project_url,
    )
    ...
    if branch != mbranch:
        logging.notice(
            'Auto resolved branch name "%s" to "%s"', branch, mbranch
        )
    branch = mbranch
    ...
    try:
        for index, (change, patch) in enumerate(zip(changes, patches)):
            ...
            if not os.path.isdir(work_dir):
                branch = _SetupWorkDirectoryForPatch(
