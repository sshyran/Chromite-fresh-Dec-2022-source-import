...
class NonClassTests(cros_test_lib.MockTestCase):
    ...
    def _TestPushChange(self, bad_cls):
        ...
        self.PatchObject(
            cros_mark_as_stable, "_DoWeHaveLocalCommits", return_value=True
        )
        self.PatchObject(
            cros_mark_as_stable.GitBranch,
            "CreateBranch",
            side_effect=side_effect,
        )
        self.PatchObject(
            cros_mark_as_stable.GitBranch, "Exists", side_effect=side_effect
        )
        ...
        cros_mark_as_stable.PushChange(
            self._branch, self._target_manifest_branch, False, "."
        )
        ...
        if not bad_cls:
            push_mock.assert_called_with(
                "merge_branch", ".", dryrun=False, staging_branch=None
            )
...
class MarkAsStableCMDTest(cros_test_lib.MockTempDirTestCase):
    ...
    def setUp(self):
        ...
        for overlay in self._overlays:
            self._git_project_overlays.setdefault(
                self._overlay_remote_ref[overlay], []
            ).append(overlay)
            self._overlay_tracking_branch[overlay] = self._overlay_remote_ref[
                overlay
            ].ref
    ...
    def testWorkOnCommit(self):
        ...
        self.PatchObject(
            cros_mark_as_stable, "_GetOverlayToEbuildsMap", return_value={}
        )
    ...
    def testCommitOverlays(self):
        ...
        self.PatchObject(
            cros_mark_as_stable.GitBranch, "Exists", return_value=True
        )
...
class MainTests(
    ...
    def setUp(self):
        ...
        self.PatchObject(
            portage_util, "FindOverlays", return_value=self._overlays
        )
        self.PatchObject(
...
class GitBranchTest(cros_test_lib.MockTestCase):
    ...
    def testCheckoutCreate(self):
        ...
        self.start_mock.assert_called_with(
    ...
    def testCheckoutNoCreate(self):
        ...
        cros_mark_as_stable.GitBranch.Checkout(self._branch)
        self.git_mock.assert_called_with(
