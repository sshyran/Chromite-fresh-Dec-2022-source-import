...
def AdjustSymbolFileSize(symbol, tempdir, file_limit):
    ...
    if file_limit and symbol.FileSize() > file_limit:
        with cros_build_lib.UnbufferedNamedTemporaryFile(
            ...
            logging.warning(
                "stripped CFI for %s reducing size %s > %s",
                symbol.display_name,
                original_file_size,
                file_size,
            )
...
def FindDuplicates(symbols, status_url, api_key, timeout=DEDUPE_TIMEOUT):
    ...
    for batch in BatchGenerator(symbols, DEDUPE_LIMIT):
        ...
        try:
            result = ExecRequest(
                "post",
                "%s/symbols:checkStatuses" % status_url,
                timeout,
                api_key=api_key,
...
def UploadSymbolFile(upload_url, symbol, api_key):
    ...
    upload = ExecRequest(
        "post", "%s/uploads:create" % upload_url, timeout, api_key
    )
    ...
    if upload and "uploadUrl" in upload.keys():
        ...
        logging.debug(
            "Executing post to uploads/complete: %s", symbol.display_name
        )
        ExecRequest(
            "post",
            "%s/uploads/%s:complete" % (upload_url, upload["uploadKey"]),
            timeout,
            api_key=api_key,
...
def PerformSymbolsFileUpload(symbols, upload_url, api_key):
    ...
    for s in symbols:
        if failures < MAX_TOTAL_ERRORS_FOR_RETRY and s.status in (
            SymbolFile.INITIAL,
            SymbolFile.ERROR,
        ):
            ...
            try:
                ...
                if s.status != SymbolFile.DUPLICATE:
                    logging.info(
                        "upload of %s with size %10i bytes took %s",
                        s.display_name,
                        s.FileSize(),
                        t,
                    )
                ...
                logging.warning(
                    "could not upload: %s: HTTP error: %s", s.display_name, e
                )
...
def ReportResults(symbols, failed_list):
    ...
    logging.info(
        "Uploaded %(uploaded)d, Skipped %(duplicate)d duplicates.",
        result_counts,
    )
...
def main(argv):
    ...
    parser.add_argument(
        "--board", default=None, help="Used to find default breakpad_root."
    )
    parser.add_argument(
        "--breakpad_root",
        type="path",
    ...
    parser.add_argument(
        "--server", type=str, default=None, help="URI for custom symbol server"
    )
    parser.add_argument(
        "--regenerate",
        action="store_true",
    ...
    parser.add_argument(
        "--upload-limit", type=int, help="only upload # number of symbols"
    )
    parser.add_argument(
        "--strip_cfi",
        type=int,
