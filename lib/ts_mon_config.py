    ...
    if fields:
        for key, val in fields.items():
            if isinstance(val, bool):
                ...
                logging.error(
                    "Couldn't classify the metric field %s:%s", key, val
                )
...
def _SetupTsMonFromOptions(options, suppress_exception):
    ...
    try:
        ...
        logging.warning(
            "Failed to configure ts_mon, monitoring is disabled: %s",
            e,
            exc_info=True,
        )
...
def _GenerateTsMonArgparseOptions(
    service_name, short_lived, auto_flush, debug_file, task_num
):
    ...
    def _CallMetric(self, message):
        ...
        try:
            ...
            getattr(metric, message.method)(
                *message.method_args, **message.method_kwargs
            )
            self.pending = True
        except Exception:
            logging.exception(
                "Caught an exception while running %s", _MethodCallRepr(message)
            )
...
def _MethodCallRepr(message):
    ...
    args_strings = [repr(x) for x in args] + [
        (str(k) + "=" + repr(v)) for k, v in kwargs.items()
    ]
