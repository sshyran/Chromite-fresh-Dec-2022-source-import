...
class ChromeosReleases(object):
    ...
    def ImageName(channel, board, version, key, image_type):
        ...
        template = (
            "chromeos_%(version)s_%(board)s_%(image_type)s"
    ...
    def UnsignedImageUri(build, milestone, image_type):
        ...
        return os.path.join(
            ChromeosReleases.BuildUri(build),
            ChromeosReleases.UnsignedImageArchiveName(
                build.board, build.version, milestone, image_type
            ),
        )
    ...
    def DLCPayloadName(
        ...
        if src_version:
            ...
            template = (
                "dlc_%(dlc_id)s_%(dlc_package)s_%(version)s_%(board)s_"
    ...
    def PayloadName(
        ...
        if src_version:
            ...
            template = (
                "chromeos_%(version)s_%(board)s_%(channel)s_"
    ...
    def ParsePayloadUri(cls, payload_uri):
        ...
        if image_values:
            ...
            return Payload(
                src_image=src_image, tgt_image=tgt_image, uri=payload_uri
            )
