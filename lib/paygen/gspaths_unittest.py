...
class GsPathsDataTest(cros_test_lib.TestCase):
    ...
    def testBuild(self):
        ...
        expected_str = (
            "Build definition (board='board-name',"
...
class GsPathsChromeosReleasesTest(cros_test_lib.TestCase):
    ...
    _GS_BUILD_PATH_TEMPLATE = (
        "gs://%(bucket)s/%(channel)s/%(board)s/%(version)s"
    )
    ...
    _GS_PAYLOADS_SIGNING_PATH_TEMPLATE = "/".join(
        (_GS_BUILD_PATH_TEMPLATE, "payloads", "signing")
    )
    ...
    def setUp(self):
        ...
        self.release_build = gspaths.Build(
    ...
    def testBuildPayloadsSigningUri(self):
        ...
        self.assertEqual(
            ...
            self._PopulateGsPath(
                self._GS_PAYLOADS_PATH_TEMPLATE, suffix="LOCK_flag"
            ),
        )
    ...
    def testParseImageUri(self):
        ...
        basic_dict["uri"] = uri_basic = (
            self._GS_IMAGE_PATH_TEMPLATE % basic_dict
        )
        ...
        expected_basic_str = gspaths.ChromeosReleases.ImageName(
            expected_basic.build.channel,
            expected_basic.build.board,
            expected_basic.build.version,
            expected_basic.key,
        ...
        expected_npo = gspaths.Image(
            build=self.release_build,
            key=self.key,
            image_type=self.signed_image_type,
            image_channel=npo_channel,
            image_version=npo_version,
        ...
        expected_npo_minios = gspaths.MiniOSImage(
            build=self.release_build,
            key=self.key,
            image_type=self.signed_image_type,
            image_channel=npo_channel,
        ...
        expected_npo_str = gspaths.ChromeosReleases.ImageName(
            expected_npo.image_channel,
            expected_npo.build.board,
            expected_npo.image_version,
            expected_npo.key,
            expected_npo.image_type,
        )
        ...
        signer_output = (
            "gs://chromeos-releases/dev-channel/link/4537.7.0/"
    ...
    def testDLCImageString(self):
        ...
        image_uri = (
            "gs://chromeos-releases/foo-channel/board-name/1.2.3/dlc/" "%s"
        ) % (dlc_image_str)
    ...
    def testParseUnsignedImageUri(self):
        ...
        expected = gspaths.UnsignedImageArchive(
            build=self.build,
            milestone=self.milestone,
    ...
    def testPayloadNamePreset(self):
        ...
        delta = gspaths.ChromeosReleases.PayloadName(
            channel=self.channel,
            board=self.board,
            version=self.version,
            key=self.key,
        ...
        self.assertEqual(
            delta, self._Populate(self._DELTA_PAYLOAD_NAME_TEMPLATE)
        )
        self.assertEqual(
            full_unsigned,
    ...
    def testPayloadNameRandom(self):
        ...
        self.assertEqual(
            full,
            self._Populate(
                self._FULL_PAYLOAD_NAME_TEMPLATE, random_str=full_random_str
            ),
        )
        ...
        self.assertEqual(
            delta,
            self._Populate(
                self._DELTA_PAYLOAD_NAME_TEMPLATE, random_str=delta_random_str
            ),
        )
    ...
    def testPayloadDLC(self):
        ...
        self.assertEqual(
            full,
            self._Populate(
                self._FULL_DLC_PAYLOAD_NAME_TEMPLATE,
                dlc_id=self.dlc_id,
                dlc_package=self.dlc_package,
                random_str=self.random_str,
            ),
    ...
    def testPayloadMiniOS(self):
        ...
        self.assertEqual(
            full, self._Populate(self._FULL_MINIOS_PAYLOAD_NAME_TEMPLATE)
        )
    ...
    def testPayloadUri(self):
        ...
        self.assertEqual(
            min_full, self._Populate(self._GS_FULL_PAYLOAD_PATH_TEMPLATE)
        )
        ...
        self.assertEqual(
            max_full,
            self._Populate(
                self._GS_FULL_PAYLOAD_PATH_TEMPLATE,
                image_channel=test_random_channel,
                image_version=test_max_version,
            ),
        )
        ...
        self.assertEqual(
            min_delta,
            self._Populate(
                self._GS_DELTA_PAYLOAD_PATH_TEMPLATE,
                src_version=test_min_version,
            ),
        )
        ...
        self.assertEqual(
            dlc_full,
            self._Populate(
                self._GS_FULL_DLC_PAYLOAD_PATH_TEMPLATE,
                src_version=test_min_version,
                image_version=test_max_version,
                image_channel=test_random_channel,
                dlc_id=self.dlc_id,
    ...
    def testParsePayloadUri(self):
        ...
        max_full_uri = self._Populate(
            self._GS_FULL_PAYLOAD_PATH_TEMPLATE,
            image_channel="image-channel",
            image_version=image_version,
        )

        max_delta_uri = self._Populate(
            self._GS_DELTA_PAYLOAD_PATH_TEMPLATE,
            image_channel="image-channel",
            image_version=image_version,
        )
