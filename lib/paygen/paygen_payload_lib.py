...
class PaygenSigner(object):
  ...
  def _Initialize(self):
    ...
    if (self._payload_build and
        self._payload_build.bucket == gspaths.ChromeosReleases.BUCKET):
      logging.info('Using GCS signer.')
      # We are using the official buckets, so sign it with official signers.
      self._signer = signer_payloads_client.SignerPayloadsClientGoogleStorage(
      ...
      if not self._private_key:
        self._private_key = os.path.join(constants.CHROMITE_DIR, 'ssh_keys',
                                         'testing_rsa')
      self._signer = signer_payloads_client.UnofficialSignerPayloadsClient(
...
class PaygenPayload(object):
  ...
  def _GetPlatformImageParams(self, image):
    ...
    with osutils.TempDir(base_dir=self.work_dir) as mount_point:
      with image_lib.LoopbackPartitions(
          image, destination=mount_point, part_ids=(constants.PART_ROOT_A,)):
    ...
    def _inner_run(cmd, response_queue):
      try:
        ...
        result = cros_build_lib.run(
            cmd, stdout=True, enter_chroot=True, stderr=subprocess.STDOUT)
  ...
  def _InsertSignaturesIntoPayload(self, payload_signatures,
    ...
    cmd = [
        ...
        '--metadata_signature_file=' + ':'.join(metadata_signature_file_names),
  ...
  def GetPayloadPropertiesMap(self, payload_path):
    ...
    if self.signer and self.signer.public_key:
      props_map['public_key'] = base64.b64encode(
          osutils.ReadFile(self.signer.public_key, mode='rb')).decode('utf-8')
...
def GeneratePayloads(in_paygen_payloads: List[PaygenPayload]) -> None:
  """Helper to generate paygen payloads.
  ...
  for signer, batched_paygen_payloads in signer_to_paygen_payloads.items():
    ...
    for paygen_payloads in (
      ...
      with chroot_util.TempDirInChroot() as root_paygen_payload_work_dir:
        ...
        paygen_payloads = parallel.RunTasksInProcessPool(
            CreatePayload,
            [(paygen_payload,) for paygen_payload in paygen_payloads])

        # Skip the paygen payloads which aren't generated.
        paygen_payloads = [
            paygen_payload for paygen_payload in paygen_payloads
            if not paygen_payload.skipped
        ]

        # Batch the paygen payload hashes here.
        paygen_payload_hashes = [
            paygen_payload.GenerateHashes()
            for paygen_payload in paygen_payloads
        ]
        # Hashes: [
        #   [hash_1, hash_2],
        ...
        paygen_payloads = parallel.RunTasksInProcessPool(
            VerifyPayload,
            [(paygen_payload,) for paygen_payload in paygen_payloads])
        _ = parallel.RunTasksInProcessPool(
            UploadResults,
            [(paygen_payload,) for paygen_payload in paygen_payloads])
