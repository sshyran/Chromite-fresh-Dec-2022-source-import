...
PAYLOAD_TYPES = [
    ...
    PAYLOAD_TYPE_FSI,
    PAYLOAD_TYPE_OMAHA,
    PAYLOAD_TYPE_MILESTONE,
    PAYLOAD_TYPE_STEPPING_STONE,
]
...
def _FilterForUnsignedImageArchives(artifacts):
    ...
    return [
        x
        for x in artifacts
        if gspaths.IsUnsignedImageArchive(x)
        or gspaths.IsUnsignedMiniOSImageArchive(x)
    ]
...
def _FilterForTest(artifacts):
    ...
    return [
        i
        for i in _FilterForUnsignedImageArchives(artifacts)
...
class PaygenBuild(object):
    ...
    def _MapToArchive(self, board, version):
        ...
        if not archive_board_candidates:
            ...
            raise ArchiveError(
                "found multiple build board names for %s: %s"
        ...
        if not archive_build_file_uri_list:
            raise ArchiveError(
                "cannot find archive build directory for %s"
    ...
    def _ValidateExpectedMiniOSBuildImages(self, build, images):
        ...
        if any(not x.minios for x in images):
            raise BuildCorrupt(
                f"{build} has unexpected MiniOS images: {images}."
            )
    ...
    def _DiscoverTestImage(self, build, os_type):
        ...
        if len(images) > 1:
            raise BuildCorrupt(
                "%s has multiple test images: %s" % (build, images)
            )
        ...
        for f in (_FilterForMp, _FilterForPremp, _FilterForTest):
            ...
            if filtered_source and filtered_target:
                assert len(filtered_source) == 1, (
                    "Unexpected: %s." % filtered_source
                )
                assert len(filtered_target) == 1, (
                    "Unexpected: %s." % filtered_target
                )
    ...
    def _DiscoverRequiredMiniOSDeltasBuildToBuild(self, source_images, images):
        ...
        for func in (_FilterForMp, _FilterForPremp, _FilterForTest):
            ...
            if filtered_source and filtered_target:
                assert (
                    len(filtered_source) == 1
                ), f"Unexpected: {filtered_source}."
                assert (
                    len(filtered_target) == 1
                ), f"Unexpected: {filtered_target}."
    ...
    def _DiscoverRequiredDLCDeltasBuildToBuild(self, source_images, images):
        ...
        for source_image in source_images:
            for image in images:
                if (
                    source_image.dlc_id == image.dlc_id
    ...
    def _DiscoverRequiredPayloads(self):
        ...
        payloads.append(
            gspaths.Payload(tgt_image=test_minios_image, minios=True)
        )

        # Add n2n test delta.
        if not self._skip_delta_payloads:
            n2n_payload = gspaths.Payload(
                tgt_image=test_image, src_image=test_image
            )
        ...
        for source in self.GetPaygenJson(
            self._build.board, self._build.channel
        ):
            ...
            logging.info(
                "Considering: %s %s", source["delta_type"], source_build
            )

            if (
                not source["generate_delta"]
                and not source["full_payload_tests"]
            ):
                logging.warning("Skipping. No payloads or tests requested.")
                continue

            if not gspaths.VersionGreater(
                self._build.version, source_build.version
            ):
    ...
    def _ShouldSign(self, image):
        ...
        return (
            ...
            or gspaths.IsDLCImage(image)
            or gspaths.IsMiniOSImage(image)
        )
    ...
    def _FindFullTestPayloads(self, channel, version):
        ...
        # Store in cache.
        self._version_to_full_test_payloads[
            (channel, version)
        ] = full_test_payloads
    ...
    def _AutotestPayloads(self, payload_tests):
        ...
        for payload_test in payload_tests:
            paygen_test_config = self._PaygenTestConfig(
                payload_test, suite_name
            )
        ...
        if cmd_result.returncode != 0:
            logging.error(
                "Error (%d) when tarring control files", cmd_result.returncode
            )
            raise PayloadTestError(
    ...
    def CreatePayloads(self):
        ...
        try:
            with gslock.Lock(lock_uri):
                ...
                if new_payloads:
                    logging.info(
                        "Generating %d new payload(s)", len(new_payloads)
                    )
                    self._GeneratePayloads(new_payloads)
                    logging.info(
                        "Finished generating payloads: %s", self._build
                    )
                else:
...
def ScheduleAutotestTests(
    suite_name, board, model, build, payload_test_configs
):
    ...
    if cmd_result.to_raise:
        if isinstance(cmd_result.to_raise, failures_lib.TestWarning):
            logging.warning(
                "Warning running test suite; error output:\n%s",
                cmd_result.to_raise,
            )
        else:
...
def _TestPlan(payload_test_configs, suite_name=None, build=None):
    ...
    for payload_test in payload_test_configs:
        ...
        shown_test_name = "%s_%s" % (
            test_name,
            payload_test.unique_name_suffix(),
        )
