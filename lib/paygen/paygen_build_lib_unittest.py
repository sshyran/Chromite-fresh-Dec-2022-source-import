...
class BasePaygenBuildLibTestWithBuilds(
    BasePaygenBuildLibTest, cros_test_lib.TempDirTestCase
):
    ...
    def testValidateExpectedBuildImages(self):
        ...
        paygen._ValidateExpectedBuildImages(
            self.target_build, (self.basic_image,)
        )
        ...
        paygen._ValidateExpectedBuildImages(
            self.target_build, (self.basic_image, self.premp_image)
        )
        ...
        with self.assertRaises(paygen_build_lib.BuildCorrupt):
            paygen._ValidateExpectedBuildImages(
                self.target_build, (self.basic_image, self.basic_image)
            )
...
class TestPaygenBuildLibTestGSSearch(BasePaygenBuildLibTestWithBuilds):
    ...
    def testFindExistingPayloads(self):
        ...
        ls_mock = self.PatchObject(
            gs.GSContext, "LS", return_value=["foo_result"]
        )
    ...
    def testDiscoverTestImages(self):
        ...
        # Run the test.
        result = paygen._DiscoverTestImage(
            self.target_build, gspaths.OSType.CROS
        )
        ...
        result_minios = paygen._DiscoverTestImage(
            self.target_build, gspaths.OSType.MINIOS
        )
    ...
    def testDiscoverRequiredDeltasBuildToBuild(self):
        ...
        self.assertCountEqual(
            ...
            [
                gspaths.Payload(
                    src_image=self.prev_image, tgt_image=self.basic_image
                ),
                gspaths.Payload(
    ...
    def testDiscoverRequiredDLCDeltasBuildToBuild(self):
        ...
        self.assertCountEqual(
            ...
            [
                gspaths.Payload(
                    src_image=self.prev_dlc_package_image,
...
class TestPaygenBuildLibDiscoverRequiredPayloads(
    MockImageDiscoveryHelper, cros_test_lib.TempDirTestCase
):
    ...
    def testCanaryEverything(self):
        ...
        premp_image_minios = self.addMiniOSSignedImage(
            target_build, key="premp"
        )
        ...
        prev_premp_image_minios = self.addMiniOSSignedImage(
        ...
        mp_full_minios = gspaths.Payload(
            ...
            uri=(
                "gs://crt/canary-channel/auron-yuna/9999.0.0/payloads/minios/"
        ...
        self.assertCountEqual(
            ...
            [
                paygen_build_lib.PayloadTest(
                    test_full, "canary-channel", "9999.0.0"
                ),
    ...
    def testCanarySkipDeltas(self):
        ...
        test_image_minios = self.addTestMiniOSImage(target_build)
        _prev_premp_image = self.addSignedImage(prev_build, key="premp")
        _prev_premp_image_minos = self.addMiniOSSignedImage(
            prev_build, key="premp"
        )
    ...
    def testStable(self):
        ...
        mp_full_minios = gspaths.Payload(
            tgt_image=mp_image_minios,
            uri=(
                "gs://crt/stable-channel/auron-yuna/9999.0.0/payloads/minios/"
    ...
    def testApplicableModels(self):
        ...
        expected_fsi_payload_test = paygen_build_lib.PayloadTest(
            **{
                "applicable_models": ["basking", "electro"],
                "src_channel": "stable-channel",
                "src_version": "11021.56.0",
                "payload_type": "FSI",
                "payload": gspaths.Payload(
                    **{
                        ...
                        "exists": False,
                        "tgt_image": gspaths.UnsignedImageArchive(
                            **{
                                "image_type": None,
                                "uri": None,
                                "build": gspaths.Build(
...
class TestCreatePayloads(BasePaygenBuildLibTestWithBuilds):
    ...
    def testCreatePayloadsSuccess(self):
        ...
        self.assertEqual(
            testdata, ("suite_name", "archive_board", "archive_build", [])
        )

        self.assertEqual(
            self.mockGenerate.call_args_list,
            [
                mock.call(payloads),
            ],
        )
        ...
        self.assertEqual(
            self.mockArchive.call_args_list,
            [
        ...
        self.assertEqual(
            self.mockAutotest.call_args_list,
            [
...
class TestAutotestPayloadsPayloads(BasePaygenBuildLibTestWithBuilds):
    ...
    def setUp(self):
        ...
        self.mockFind = self.PatchObject(
            ...
            side_effect=lambda channel, version: [
                "%s_%s_uri" % (channel, version)
            ],
        )
