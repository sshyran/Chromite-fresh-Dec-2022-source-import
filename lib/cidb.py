...
def _IsRetryableException(e):
    ...
    if "OperationalError" in str(type(e)):
        ...
        if encountered_error_codes & _RETRYABLE_OPERATIONAL_ERROR_CODES:
            ...
            log.info(
                "None of error codes encountered %s are-retryable.",
...
SqlConnectionRetryArgs = collections.namedtuple(
    "SqlConnectionRetryArgs", ("max_retry", "sleep", "backoff_factor")
)
...
class SchemaVersionedMySQLConnection(object):
    ...
    def _UpdateConnectArgs(self, db_credentials_dir, for_service=False):
        ...
        self._UpdateConnectUrlArgs(
            "password", db_credentials_dir, "password.txt"
        )
    ...
    def __init__(
        ...
        if not sqlalchemy_imported:
            raise AssertionError(
                "Unable to open cidb connections, as sqlalchemy "
        ...
        databases = self._ExecuteWithEngine(
            "SHOW DATABASES", temp_engine
        ).fetchall()
        ...
        log.info(
            "Created a SchemaVersionedMySQLConnection, "
            "sqlalchemy version %s",
    ...
    def _GetMigrationScripts(self):
        ...
        migration_scripts = glob.glob(
            os.path.join(self.db_migrations_dir, "*.sql")
        )
        migrations = []
        ...
        for (number, script) in migrations:
            ...
            if number > self.schema_version:
                ...
                if self.schema_version != number:
                    raise DBException(
                        "Migration script %s did not update "
    ...
    def _GetPrimaryKey(self, table):
        ...
        if len(key_columns) != 1:
            raise DBException(
                "Table %s does not have a 1-column primary " "key." % table
            )
    ...
    def _ExecuteWithEngine(self, query, engine, *args, **kwargs):
        ...
        log.info(
            "Running cidb query on pid %s, str(query) starts with %s",
            os.getpid(),
            str(query)[:100],
        )
    ...
    def _GetEngine(self):
        ...
        if pid == self._engine_pid and self._engine:
            ...
            e = sqlalchemy.create_engine(
                self._connect_url, listeners=[self._listener_class()]
            )
            self._engine = e
            self._engine_pid = pid
            log.info(
                "Created cidb engine %s@%s for pid %s",
                e.url.username,
                e.url.host,
                pid,
            )
  ...
  """
    ...
    def InsertChildConfigPerBuild(self, build_id, child_config):
        ...
        self._Insert(
            ...
            {"build_id": build_id, "child_config": child_config},
        )
    ...
    def FinishBuildStage(self, build_stage_id, status):
        ...
        return self._Update(
            "buildStageTable",
            build_stage_id,
            {"status": status, "finish_time": current_timestamp, "final": True},
        )
    ...
    def FinishBuild(
        ...
        current_timestamp = sqlalchemy.func.current_timestamp()
        values = {"finish_time": current_timestamp, "final": True}
    ...
    def GetBuildStatusesWithBuildbucketIds(self, buildbucket_ids):
        status = self._SelectWhere(
            "buildTable",
            "buildbucket_id IN (%s)"
            % ",".join([str(int(x)) for x in buildbucket_ids]),
    ...
    def GetBuildsStages(self, build_ids):
        ...
        query = (
            "SELECT %s, %s FROM buildStageTable bs JOIN "
            "buildTable b ON bs.build_id=b.id"
            % (", ".join(bs_prepended_columns), ", ".join(b_prepended_columns))
        )
    ...
    def GetBuildsHistory(
        ...
        if buildbot_generation is not None:
            where_clauses.append(
                'buildbot_generation = "%s"' % buildbot_generation
            )
        if final:
        ...
        results = self._Execute(query).fetchall()
        return [
            dict(zip(CIDBConnection.BUILD_STATUS_KEYS, values))
    ...
    def _GetBuildMessagesWithClause(self, clause):
        ...
        results = self._Execute(
            "%s WHERE %s" % (self._SQL_FETCH_MESSAGES, clause)
        ).fetchall()
        return [dict(zip(columns, values)) for values in results]
...
class CIDBConnectionFactoryClass(factory.ObjectFactory):
    ...
    def _allowed_cidb_transition(self, from_setup, to_setup):
        ...
        if (
            from_setup == CONNECTION_TYPE_INV
            and to_setup == CONNECTION_TYPE_MOCK
        ):
            return True
