        ...
        def ReverseTree(packages):
            ...
            for pkg in packages:
                ...
                if emerge_pkg and emerge_pkg.type_name == "binary":
                    ...
                    defined_binpkg_phases = binpkg_phases.intersection(
                        defined_phases
                    )
                ...
                for dep, dep_item in packages[pkg]["deps"].items():
                    ...
                    if unknown_dep_types:
                        ...
                        print(
                            "  %s -> %s (%s)"
                            % (pkg, dep, "/".join(unknown_dep_types))
                        )
                        sys.exit(1)
                ...
                for dep in deps_map[pkg]["needs"]:
                    if dep in unresolved:
                        ...
                        for i in range(len(mycycle) - 1):
                            ...
                            cycles.setdefault(pkg1, {}).setdefault(
                                pkg2, mycycle
                            )
...
DepgraphResult = collections.namedtuple(
    "DepgraphResult", ("deps", "bdeps", "packages")
)


def _get_emerge_args(
    sysroot_path: str, packages: PackageCollection, include_bdeps: bool
) -> List[str]:
...
def _get_sysroot_path(
    ...
    build_target: Union[str, "build_target_lib.BuildTarget"],
) -> str:
...
def get_sdk_dependency_graph(
    pkgs: Optional[PackageCollection] = None, with_src_paths: bool = False
...
def get_sysroot_dependency_graph(
    ...
    return _create_graph_from_deps(
        result.deps, sysroot, result.packages, with_src_paths
    )
