...
_GERRIT_CHANGE_ID_TOTAL_LENGTH = _GERRIT_CHANGE_ID_LENGTH + len(
    _GERRIT_CHANGE_ID_PREFIX
)
...
def ParseChangeID(text, error_ok=True):
    ...
    valid = (
        ...
        and len(text) == _GERRIT_CHANGE_ID_TOTAL_LENGTH
        and git.IsSHA1(text[len(_GERRIT_CHANGE_ID_PREFIX) :].lower())
    )
...
def ParseFullChangeID(text, error_ok=True):
    ...
    if (
        not REPO_NAME_RE.match(project)
        or not BRANCH_NAME_RE.match(branch)
        or not ParseChangeID(change_id)
    ):
        if not error_ok:
            raise ValueError("%s is not a valid full change-ID" % (text,))
...
class EbuildConflict(ApplyPatchException):
    ...
    def ShortExplanation(self):
        return (
            "deletes an ebuild that is not present anymore. For this reason, "
...
class ChangeMatchesMultipleCheckouts(PatchException):
    ...
    def ShortExplanation(self):
        return (
            "matches multiple checkouts. Does the manifest check out the "
...
def ParsePatchDep(
    ...
    no_change_id=False,
    no_sha1=False,
    no_full_change_id=False,
    no_gerrit_number=False,
):
    ...
    if parsed:
        if no_full_change_id:
            raise ValueError(
                "ParsePatchDep: Full Change-ID is not allowed: %r."
                % original_text
            )
    ...
    if parsed:
        if no_gerrit_number:
            raise ValueError(
                "ParsePatchDep: Gerrit number is not allowed: %r."
                % original_text
            )
...
def GetPaladinDeps(commit_message):
    ...
    PALADIN_DEPENDENCY_RE = re.compile(
        r"^([ \t]*CQ.?DEPEND.)(.*)$", re.MULTILINE | re.IGNORECASE
    )
    PATCH_RE = re.compile("[^, ]+")
...
class PatchQuery(object):
    ...
    def __init__(
        ...
        self.gerrit_number = (
            None if gerrit_number is None else ParseGerritNumber(gerrit_number)
        )
    ...
    def _SetFullChangeID(self):
        ...
        if (
            ...
            and self.tracking_branch is not None
            and self.change_id is not None
        ):
...
class GitRepoPatch(PatchQuery):
    ...
    pattern = (
        r"^"
        + re.escape(_GERRIT_CHANGE_ID_PREFIX)
        + r"[0-9a-fA-F]{"
        + re.escape(str(_GERRIT_CHANGE_ID_LENGTH))
        + r"}$"
    )
    _STRICT_VALID_CHANGE_ID_RE = re.compile(pattern)
    _GIT_CHANGE_ID_RE = re.compile(
        r"^Change-Id:[\t ]*(\w+)\s*$", re.I | re.MULTILINE
    )
    ...
    def _PullData(self, rev, git_repo):
        ...
        output = log.split("\0")
        if len(output) != 6:
            raise git.GitException(
                "Git did not format log data in expected format."
            )
    ...
    def Fetch(self, git_repo):
        ...
        if sha1 is None:
            git.RunGit(
                git_repo,
                ["fetch", "-f", self.project_url, self.ref],
                print_cmd=True,
            )
    ...
    def _AmendCommitMessage(self, git_repo):
        ...
        self.sha1 = ParseSHA1(
            self._PullData("HEAD", git_repo)[0], error_ok=False
        )
    ...
    def CherryPick(
        ...
        try:
            ...
            if ret not in (1, 2):
                logging.error(
                    "Unknown cherry-pick exit code %s; %s", ret, error
                )
                raise ApplyPatchException(
                    self,
                    inflight=inflight,
                    message=(
                        "Unknown exit code %s returned from cherry-pick "
                        "command: %s" % (ret, error)
                    ),
                )
            elif ret == 1:
            ...
            if not trivial:
                ...
                logging.error(
                    'If the git error is "unable to read tree", '
            ...
            if reset_target is not None:
                git.RunGit(
                    git_repo, ["reset", "--hard", reset_target], check=False
                )
    ...
    def Apply(self, git_repo, upstream, revision=None, trivial=False):
        ...
        return self._ApplyHelper(
            git_repo, upstream, trivial, inflight, use_merge
        )
    ...
    def _ApplyHelper(
        ...
        try:
            ...
            if do_checkout:
                git.RunGit(
                    git_repo,
                    ["checkout", "-f", constants.PATCH_BRANCH],
                    check=False,
                )
    ...
    def _ValidateMergeCommit(self, git_repo, upstream, parents):
        ...
        parent_is_ancestor = parent1._IsAncestorOf(
            git_repo, upstream_as_patch
        ) or parent2._IsAncestorOf(git_repo, upstream_as_patch)
    ...
    def _ParseChangeId(self, data):
        ...
        if not change_id_match:
            raise BrokenChangeID(
                self, "Missing Change-Id in %s" % (data,), missing=True
            )
    ...
    def _FindEbuildConflicts(self, git_repo, upstream, inflight=False):
        ...
        ebuilds = [
            path
            for (path, mtype) in self.GetDiffStatus(git_repo).items()
...
class LocalPatch(GitRepoPatch):
    ...
    def _GetCarbonCopy(self):
        ...
        result = git.Log(
            self.project_url, format=format_string, max_count=1, rev=self.sha1
        )
        lines = result.splitlines()
        field_value = dict(
            zip([name for name, _ in fields], [line.strip() for line in lines])
        )
    ...
    def Upload(
        ...
        result = git.RunGit(
            self.project_url, cmd, stdout=True, stderr=subprocess.STDOUT
        )
...
class GerritFetchOnlyPatch(GitRepoPatch):
    ...
    def FromAttrDict(cls, attr_dict):
        ...
        return GerritFetchOnlyPatch(
            ...
            total_fail_count=int(attr_dict[ATTR_TOTAL_FAIL_COUNT]),
...
class GerritPatch(GerritFetchOnlyPatch):
    ...
    def __init__(self, patch_dict, remote, url_prefix):
        ...
        if "currentPatchSet" in self.patch_dict:
            self._approvals = self.patch_dict["currentPatchSet"].get(
                "approvals", []
            )
    ...
    def ConvertQueryResults(change, host):
        ...
        try:
            ...
            current_revision_info = change.get("revisions", {}).get(
                current_revision
            )
            if current_revision_info:
                ...
                if current_commit:
                    ...
                    patch_dict["dependsOn"] = [
                        {"revision": p["commit"]} for p in parents
                    ]
    ...
    def HasApproval(self, field, value):
        ...
        type_approvals = [
            x["value"] for x in self._approvals if x["type"] == field
        ]
...
def FooterForApproval(approval, footers):
    ...
    if approval["type"] not in FOOTER_TAGS_BY_APPROVAL_TYPE:
        logging.warning(
            "unknown gerrit type %s (%r)", approval["type"], approval
        )
        return
...
def GeneratePatchesFromRepo(
    git_repo, project, tracking_branch, branch, remote, allow_empty=False
):
...
def _CheckLocalPatches(manifest, local_patches):
    ...
    for patch in local_patches:
        ...
        if local_branch and git.DoesCommitExistInRepo(
            project_dir, local_branch
        ):
...
def PrepareLocalPatches(manifest, patches):
    ...
    for patch in _CheckLocalPatches(manifest, patches):
        ...
        for checkout in manifest.FindCheckouts(project):
            ...
            project_patch_info.extend(
                GeneratePatchesFromRepo(
