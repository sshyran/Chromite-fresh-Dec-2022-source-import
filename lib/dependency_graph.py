    ...
    def __eq__(self, other: Union["PackageNode", package_info.PackageInfo]):
        if isinstance(other, PackageNode):
            return (
                ...
                and self._deps == set(other.dependencies)
                and self._rev_deps == set(other.reverse_dependencies)
            )
...
class DependencyGraph:
    ...
    def __init__(
        ...
        self.sysroot_path = (
            given_sysroot if given_sysroot in self._roots else None
        )
        ...
        for pkg in [package_info.parse(p) for p in root_packages]:
            self._root_package_nodes.extend(
                self.get_nodes([pkg], RootType.SYSROOT)
            )

    def __contains__(
        self, item: Union[str, package_info.PackageInfo, PackageNode]
    ) -> bool:
        """Check if the given package is in the depgraph."""
        if isinstance(item, PackageNode):
            return (
                item.atom in self._atom_list
                and item in self._atom_list[item.atom]
            )
    ...
    def _get_nodes(
        self, pkg_info: package_info.PackageInfo, root_type: RootType
    ):
        ...
        if pkg_info.cpvr in self._pkg_dict:
            ...
            return [
                x for x in self._atom_list[pkg_info.atom] if x.root in roots
            ]
        else:
            return []
    ...
    def is_dependency(
        ...
        if not dep_pkg_nodes or not src_pkg_nodes:
            ...
            subgraph = DependencyGraph(
                src_pkg_nodes, self.sysroot_path, [src_pkg]
            )
