...
"""
                % "\n".join(
                    ["  cros_chroot_%03d\t/dev/any" % i for i in range(1000)]
                )
            )
            vg = cros_sdk_lib.FindVolumeGroupForDevice("/chroot", "")
...
class TestMountChroot(cros_test_lib.MockTempDirTestCase):
    ...
    def testFromScratch(self):
        ...
        with cros_test_lib.RunCommandMock() as rc_mock:
            rc_mock.AddCmdResult(
                self._VGS_LOOKUP, returncode=self._LVM_FAILURE_CODE
            )
            rc_mock.AddCmdResult(self._VGCREATE, stdout="")
            rc_mock.AddCmdResult(
                self._LVS_LOOKUP, returncode=self._LVM_FAILURE_CODE
            )
    ...
    def testMissingMount(self):
        ...
        with cros_test_lib.RunCommandMock() as rc_mock:
            rc_mock.AddCmdResult(
                self._VGS_LOOKUP, returncode=self._LVM_SUCCESS_CODE
            )
            rc_mock.AddCmdResult(self._VGCHANGE)
            rc_mock.AddCmdResult(
                self._LVS_LOOKUP, returncode=self._LVM_SUCCESS_CODE
            )
            rc_mock.AddCmdResult(self._MOUNT)
    ...
    def testExistingChroot(self):
        ...
        osutils.Touch(
            os.path.join(self.chroot_path, "etc", "cros_chroot_version"),
            makedirs=True,
        )
...
class TestCleanupChrootMount(cros_test_lib.MockTempDirTestCase):
    ...
    def testUnmountedCleanup(self):
        ...
        with cros_test_lib.RunCommandMock() as rc_mock:
            ...
            rc_mock.AddCmdResult(
                self._VGS_VG_LOOKUP, returncode=self._LVM_SUCCESS_CODE
            )
            rc_mock.AddCmdResult(self._VGCHANGE_N)
            rc_mock.AddCmdResult(self._LOSETUP_DETACH)
    ...
    def testDevOnlyCleanup(self):
        ...
        with cros_test_lib.RunCommandMock() as rc_mock:
            rc_mock.AddCmdResult(self._LOSETUP_FIND, stdout="/dev/loop1")
            rc_mock.AddCmdResult(
                self._VGS_VG_LOOKUP, returncode=self._LVM_FAILURE_CODE
            )
            rc_mock.AddCmdResult(self._LOSETUP_DETACH)
