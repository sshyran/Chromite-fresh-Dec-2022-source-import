...
class Downloader(object):
    ...
    def ListBuildDir(self):
        ...
        output_format = (
            "Name: %(name)s Accessed: %(accessed)s "
    ...
    def _DownloadArtifactsInBackground(self, artifacts):
        ...
        """
        logging.debug(
            "Invoking background download of artifacts for %r", artifacts
        )
        thread = threading.Thread(
            target=self._DownloadArtifactsSerially, args=(artifacts, False)
        )
        thread.start()
    ...
    def Wait(self, name, is_regex_name, alt_name, timeout):
        ...
        try:
            ...
            if common_util.IsAnonymousCaller(e) and alt_name:
                ...
                names = [
                    n.format(
                        build=self._build, board=self._board.replace("-", "_")
                    )
                    for n in alt_name
                ]
        if not names:
