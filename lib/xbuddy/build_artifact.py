...
class Artifact(object, metaclass=ArtifactMeta):
    ...
    def __init__(
        ...
        exception_file_name = (
            "." + self._SanitizeName(build) + self.marker_name + ".exception"
        )
        # The exception file needs to be located in parent folder, since the
        # install_dir will be deleted is the build does not exist.
        self.exception_file_path = os.path.join(
            os.path.dirname(install_dir), exception_file_name
        )
    ...
    def _SanitizeName(name):
        ...
        """
        return (
            name.replace("*", "STAR").replace(".", "DOT").replace("/", "SLASH")
        )
    ...
    def ArtifactStaged(self):
        ...
        if self.store_installed_files:
            ...
            missing_files = [
                fname for fname in files if not os.path.exists(fname)
            ]
            if missing_files:
                logging.error(
                    "***ATTENTION*** %s files listed in %s are missing:\n%s",
            ...
            if not self.ArtifactStaged():
                ...
                if self.optional_name:
                    try:
                        ...
                        logging.warning(
                            "Unable to download %s; fall back to download %s",
                            self.optional_name,
                            self.name,
                        )
                    else:
                ...
                try:
                    ...
                    for filename in files:
                        ...
                        self.install_path = downloader.Fetch(
                            filename, real_install_dir
                        )
                    self._Setup()
...
class MultiArtifact(Artifact):
    ...
    def _Setup(self):
        ...
        self.installed_files = [
            os.path.join(self.install_dir, self.install_subdir, name)
...
class BundledArtifact(Artifact):
    ...
    def StagedFiles(self):
        ...
        if self._files_to_extract:
            return [
                x
                for x in files
                if os.path.relpath(x, self.install_dir)
                in self._files_to_extract
            ]
        return files
    ...
    def _ExtractZipfile(self):
        ...
        file_list = [
            os.path.join(self.install_dir, line[30:].strip())
...
class AutotestTarball(BundledArtifact):
    ...
    def _Setup(self):
        ...
        autotest_dir = os.path.join(
            self.install_dir, devserver_constants.AUTOTEST_DIR
        )
        ...
        if not os.path.exists(
            os.path.join(autotest_pkgs_dir, "packages.checksum")
        ):
            cmd = [
...
class SignedArtifact(Artifact):
    ...
    def _Setup(self):
        ...
        install_path = os.path.join(
            self.install_dir, self.install_subdir, self.name
        )
...
_AddCrOSArtifact(
    ...
    MultiArtifact,
    r"chromeos_.*_full_dev.*bin(\.json)?\Z",
    is_regex_name=True,
    alt_name=[
        "chromeos_{build}_{board}_dev.bin",
        "chromeos_{build}_{board}_dev.bin.json",
    ],
)
_AddCrOSArtifact(
    artifact_info.DELTA_PAYLOAD,
    MultiArtifact,
    r"chromeos_.*_delta_dev.*bin(\.json)?\Z",
    is_regex_name=True,
)
_AddCrOSArtifact(
    artifact_info.STATEFUL_PAYLOAD, Artifact, devserver_constants.STATEFUL_FILE
)
_AddCrOSArtifact(
...
_AddCrOSArtifact(
    artifact_info.SIGNED_IMAGE, SignedArtifact, SIGNED_RECOVERY_IMAGE_FILE
)
_AddCrOSArtifact(
...
_AddCrOSArtifact(
    artifact_info.CONTROL_FILES, BundledArtifact, CONTROL_FILES_FILE
)
_AddCrOSArtifact(
    artifact_info.AUTOTEST_PACKAGES, AutotestTarball, AUTOTEST_PACKAGES_FILE
)
