...
class BuildSummaryTests(cros_test_lib.TestCase):
    ...
    def testPartialJson(self):
        ...
        self.assertEqual(
            summary.master_build_id, default_summary.master_build_id
        )
    ...
    def testJsonNoUnecessaryKeys(self):
        ...
        self.assertSetEqual(
            set(out_fields.keys()), set(["build_number", "status"])
        )
    ...
    def testAttributesListMatchObject(self):
        ...
        self.assertSetEqual(
            set(vars(summary).keys()), set(summary._PERSIST_ATTRIBUTES)
        )
