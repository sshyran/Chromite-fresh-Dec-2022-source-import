...
class EnforcedCleanupSection(cros_build_lib.PrimaryPidContextManager):
    ...
    def ForkWatchdog(self):
        if self._forked:
            raise RuntimeError(
                "ForkWatchdog was invoked twice for %s" % (self,)
            )
        ...
        try:
            ...
            print(
                "EnforcedCleanupSection %s excepted(%r) attempting "
        ...
        cros_build_lib.PrimaryPidContextManager.ALTERNATE_PRIMARY_PID = (
            os.getpid()
        )

        raise RuntimeError(
            "Parent exited uncleanly; forcing cleanup code to run."
        )
