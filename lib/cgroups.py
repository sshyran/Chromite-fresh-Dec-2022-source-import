...
class Cgroup(object):
    ...
    def InitSystem(cls):
        ...
        def _EnsureMounted(mnt, args):
            ...
            with locking.FileLock(lock_path, "cgroup lock") as lock:
                ...
                cros_build_lib.sudo_run(
                    ["mount"] + args + [mnt], print_cmd=False
                )
        ...
        return _EnsureMounted(
            cls.MOUNT_ROOT, mount_root_args
        ) and _EnsureMounted(cls.CGROUP_ROOT, cgroup_root_args)
    ...
    def __init__(
        ...
        if _is_root:
            ...
            if parent is None:
                raise ValueError(
                    "Either _is_root must be set to True, or parent must "
                    "be non null"
                )
            if namespace in (".", ""):
                raise ValueError(
                    "Invalid namespace %r was given" % (namespace,)
                )
    ...
    def _LimitName(self, name, for_path=False, multilevel=False):
        ...
        if not path.startswith(fake_path + "/"):
            raise ValueError(
                "Name %s tried descending through this namespace into"
    ...
    def nested_groups(self):
        ...
        targets = [
            self.AddGroup(x, lazy_init=True, _overwrite=False) for x in targets
        ]
    ...
    def AddGroup(self, name, **kwargs):
        ...
        for chunk in chunks[:-1]:
            node = node._AddSingleGroup(
                chunk, parent=node, autoclean=autoclean_parents, **kwargs
            )
        return node._AddSingleGroup(
            chunks[-1], parent=node, autoclean=autoclean, **kwargs
        )
    ...
    def _RemoveGroupOnDisk(cls, path, strict, sudo_strict=True):
        ...
        if not path.startswith(cls.CGROUP_ROOT):
            raise RuntimeError(
                "cgroups.py: Was asked to wipe path %s, refusing. "
    ...
    def KillProcesses(
        self, poll_interval=0.05, remove=False, sigterm_timeout=10
    ):
        ...
        while time.time() < time_end:
            ...
            if self_kill:
                raise Exception(
                    "Bad API usage: asked to kill cgroup %s, but "
                    "current pid %s is in that group.  Effectively "
                    "asked to kill ourselves." % (self.namespace, self_kill)
                )
    ...
    def _FindCurrentCrosGroup(cls, pid=None):
        ...
        if pid is None:
            ...
            raise ValueError(
                "pid must be None, or an integer/long.  Got %r" % (pid,)
            )
...
class ContainChildren(cros_build_lib.PrimaryPidContextManager):
    ...
    def _enter(self):
        ...
        self.child = self.node.AddGroup(
            pool_name, autoclean=True, lazy_init=True
        )
        try:
    ...
    def _exit(self, exc_type, exc, exc_tb):
        with signals.DeferSignals():
            ...
            if self.run_kill:
                self.child.KillProcesses(
                    remove=True, sigterm_timeout=self.sigterm_timeout
                )
            else:
...
_cros_node = _root_node.AddGroup(
    "cros", autoclean=False, lazy_init=True, _overwrite=False
)
