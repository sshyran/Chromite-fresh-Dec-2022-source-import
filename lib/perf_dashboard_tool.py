...
def GetParser():
    ...
    group.add_argument(
        "-m", "--master", default=MASTER_NAME, help="The buildbot master field"
    )
    group.add_argument(
        "-b",
        "--bot",
    ...
    group.add_argument(
        "--cros-version", default=None, help="Chrome OS version (X.Y.Z)"
    )
    group.add_argument(
        "--chrome-version", default=None, help="Chrome version (M.X.Y.Z)"
    )

    group = parser.add_argument_group("Data Options")
    group.add_argument(
        "-t", "--test", default=TEST_NAME, help="The test name field"
    )
    group.add_argument(
        "--higher-is-better",
        default=False,
    ...
    group.add_argument(
        "-g", "--graph", help="Graph name (to group multiple tests)"
    )
    group.add_argument(
        "-d", "--description", default="data", help="Name for this data series"
    )
    group.add_argument(
        "--stdio-uri", help="Custom log page to link data point to"
    )
...
def main(argv):
    ...
    view_url = os.path.join(
        opts.url, "report?%s" % urllib.parse.urlencode(args)
    )
    logging.info("View results at %s", view_url)
    logging.info(
        "Note: To make tests public, visit %s",
        os.path.join(opts.url, "change_internal_only"),
    )
    logging.info(
        "Note: To update the test list, visit %s",
