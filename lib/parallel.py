...
class _BackgroundTask(multiprocessing.Process):
    ...
    def _DumpDebugPid(cls, log_level: int, pid: str) -> None:
        ...
        stdin = "\n".join(
            ["echo \\n>>> %s\\n\n%s" % (x, x) for x in cls.GDB_COMMANDS]
        )
        cmd = (
            "gdb",
            "--nx",
    ...
    def Wait(self):
        ...
        try:
            ...
            with open(self._output.name, "rb") as output:
                pos = 0
                running, exited_cleanly, task_errors, run_errors = (
                    True,
                    False,
                    [],
                    [],
    ...
    def start(self):
        ...
        if self.SILENT_TIMEOUT < self.MINIMUM_SILENT_TIMEOUT:
            raise AssertionError(
                "Maximum recursion depth exceeded in %r" % self
            )
...
def RunParallelSteps(
    steps, max_parallel=None, halt_on_error=False, return_values=False
):
    """Run a list of functions in parallel.
    ...
    with cros_build_lib.ContextManagerStack() as stack:
        ...
        with _BackgroundTask.ParallelTasks(
            full_steps, max_parallel=max_parallel, halt_on_error=halt_on_error
        ):
            pass
...
def BackgroundTaskRunner(task, *args, **kwargs):
    ...
    with cros_build_lib.ContextManagerStack() as stack:
        ...
        child = functools.partial(
            _BackgroundTask.TaskRunner,
            queue,
            task,
            onexit=onexit,
            task_args=args,
...
def RunTasksInProcessPool(task, inputs, processes=None, onexit=None):
    ...
    with Manager() as manager:
        ...
        with BackgroundTaskRunner(
            fn, queue=queue, processes=processes, onexit=onexit
        ) as queue:
            for idx, input_args in enumerate(inputs):
                queue.put((idx, input_args))

        return [
            x[1] for x in sorted(out_queue.get() for _ in range(len(inputs)))
        ]
