...
class FileTypeDecoderTest(cros_test_lib.TempDirTestCase):
    ...
    def testSpecialFiles(self):
        ...
        self.assertEqual(
            "inode/directory", filetype.FileTypeDecoder.DecodeFile(somedir)
        )

        a_fifo = os.path.join(self.tempdir, "a_fifo")
        os.mknod(a_fifo, stat.S_IFIFO)
        self.assertEqual(
            "inode/special", filetype.FileTypeDecoder.DecodeFile(a_fifo)
        )

        empty_file = os.path.join(self.tempdir, "empty_file")
        osutils.WriteFile(empty_file, "")
        self.assertEqual(
            "inode/empty", filetype.FileTypeDecoder.DecodeFile(empty_file)
        )
    ...
    def testTextShebangFiles(self):
        ...
        self.assertEqual(
            "text/shebang", filetype.FileTypeDecoder.DecodeFile(shebang)
        )
        ...
        self.assertEqual(
            "text/script", filetype.FileTypeDecoder.DecodeFile(script)
        )

        bash_script = os.path.join(self.tempdir, "bash_script")
        osutils.WriteFile(
            bash_script, "#!/bin/bash --debug\n# Copyright\nexit 42\n"
        )
        self.assertEqual(
            "text/script/bash", filetype.FileTypeDecoder.DecodeFile(bash_script)
        )

        pyscript = os.path.join(self.tempdir, "pyscript")
        osutils.WriteFile(
            pyscript, "#!/usr/bin/env PYTHONPATH=/foo python-2.7 -3\n# foo\n"
        )
        self.assertEqual(
            "text/script/python", filetype.FileTypeDecoder.DecodeFile(pyscript)
        )
...
""",
        ...
        self.assertEqual(
            "text/pem/cert", filetype.FileTypeDecoder.DecodeFile(some_cert)
        )
...
""",
    ...
    def testBinaryELFFiles(self):
        ...
        self.assertEqual(
            "binary/elf/dynamic-so",
            filetype.FileTypeDecoder.DecodeFile(liba_so),
        )

        prog = os.path.join(self.tempdir, "prog")
        unittest_lib.BuildELF(
            prog, undefined_symbols=["func_a"], used_libs=["a"], executable=True
        )
        self.assertEqual(
            "binary/elf/dynamic-bin", filetype.FileTypeDecoder.DecodeFile(prog)
        )
    ...
    def testBinaryCompressedFiles(self):
        ...
        osutils.WriteFile(
            ...
            b"\x00x\xad\xdb\xd1\x05\x00\x00\x00",
            mode="wb",
        )
        self.assertEqual(
    ...
    def testBinaryMiscFiles(self):
        ...
        self.assertEqual(
            "binary/tzfile", filetype.FileTypeDecoder.DecodeFile(some_timezone)
        )
