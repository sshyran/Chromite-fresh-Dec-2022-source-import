...
class GceContext(object):
    ...
    def __init__(self, project, zone, credentials, thread_safe=False):
        ...
        def _BuildRequest(http, *args, **kwargs):
            ...
            return self._BuildRetriableRequest(
                self.RETRIES, http, thread_safe, credentials, *args, **kwargs
            )

        self.gce_client = build(
            "compute",
            "v1",
    ...
    def CreateAddress(self, name, region=None):
        ...
        operation = (
            ...
            .insert(
                project=self.project, region=region or self.region, body=body
            )
            .execute()
        )
        ...
        address = (
            self.gce_client.addresses()
            .get(
                project=self.project, region=region or self.region, address=name
            )
            .execute()
        )
    ...
    def GetZoneRegion(self, zone=None):
        ...
        zone_resource = (
            ...
            .get(project=self.project, zone=zone or self.zone)
            .execute()
        )
    ...
    def CreateInstance(
        ...
        **kwargs,
        ...
        machine_type = "zones/%s/machineTypes/%s" % (
            zone or self.zone,
            machine_type or self._DEFAULT_MACHINE_TYPE,
        )
        service_accounts = (
            (
                {
        ...
        if static_address is not None:
            config["networkInterfaces"][0]["accessConfigs"][0][
                "natIP"
            ] = static_address
        if subnet is not None:
            region = self.GetZoneRegion(zone)
            config["networkInterfaces"][0][
                "subnetwork"
            ] = "regions/%s/subnetworks/%s" % (region, subnet)
        operation = (
            self.gce_client.instances()
            .insert(project=self.project, zone=zone or self.zone, body=config)
            .execute()
        )
    ...
    def DeleteInstance(self, name, zone=None):
        ...
        operation = (
            self.gce_client.instances()
            .delete(project=self.project, zone=zone or self.zone, instance=name)
            .execute()
        )
        self._WaitForZoneOperation(
    ...
    def StartInstance(self, name, zone=None):
        ...
        operation = (
            self.gce_client.instances()
            .start(project=self.project, zone=zone or self.zone, instance=name)
            .execute()
        )
        self._WaitForZoneOperation(
            operation["name"], timeout_sec=self.INSTANCE_OPERATIONS_TIMEOUT_SEC
    ...
    def StopInstance(self, name, zone=None):
        ...
        operation = (
            self.gce_client.instances()
            .stop(project=self.project, zone=zone or self.zone, instance=name)
            .execute()
        )
        self._WaitForZoneOperation(
            operation["name"], timeout_sec=self.INSTANCE_OPERATIONS_TIMEOUT_SEC
    ...
    def CreateImage(self, name, source):
        ...
        operation = (
            self.gce_client.images()
            .insert(project=self.project, body=config)
            .execute()
        )
        self._WaitForGlobalOperation(
            operation["name"],
    ...
    def DeleteImage(self, name):
        ...
        operation = (
            self.gce_client.images()
            .delete(project=self.project, image=name)
            .execute()
        )
        self._WaitForGlobalOperation(
            operation["name"], timeout_sec=self.IMAGE_OPERATIONS_TIMEOUT_SEC
        )
    ...
    def ListInstances(self, zone=None):
        ...
        result = (
            self.gce_client.instances()
            .list(project=self.project, zone=zone or self.zone)
            .execute()
        )
    ...
    def GetImage(self, image):
        ...
        try:
            return (
                self.gce_client.images()
                .get(project=self.project, image=image)
                .execute()
            )
        except HttpError as e:
            if e.resp.status == 404:
                raise ResourceNotFoundError(
                    'Image "%s" for project "%s" was not found.'
                    % (image, self.project)
                )
            else:
                raise
    ...
    def SetCommonInstanceMetadata(self, key, value):
        ...
        operation = (
            self.gce_client.projects()
            .setCommonInstanceMetadata(project=self.project, body=metadata)
            .execute()
        )
    ...
    def _WaitForZoneOperation(
        self, operation, zone=None, timeout_sec=None, timeout_handler=None
    ):
        ...
        self._WaitForOperation(
            operation, get_request, timeout_sec, timeout_handler=timeout_handler
        )

    def _WaitForRegionOperation(
        self, operation, region, timeout_sec=None, timeout_handler=None
    ):
    ...
    def _WaitForGlobalOperation(
        self, operation, timeout_sec=None, timeout_handler=None
    ):
