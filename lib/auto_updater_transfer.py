...
class Transfer(object, metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        if payload_mode not in ("scp", "parallel"):
            raise ValueError(
                "The given value %s for payload mode is not valid."
                % payload_mode
            )
...
class LocalTransfer(Transfer):
    ...
    def CheckPayloads(self):
        """Verify that all required payloads are in |self.payload_dir|."""
        logging.debug(
            "Checking if payloads have been stored in directory %s...",
            self._payload_dir,
        )
        filenames = []
        ...
        for fname in filenames:
            ...
            if not os.path.exists(payload):
                raise ChromiumOSTransferError(
                    "Payload %s does not exist!" % payload
                )
    ...
    def _TransferUpdateUtilsPackage(self):
        ...
        self._device.CopyToWorkDir(
            source_dir, mode=_SCP, log_output=True, **self._cmd_kwargs
        )
...
class LabEndToEndPayloadTransfer(Transfer):
    ...
    def _RemoteDevserverCall(self, cmd, stdout=False):
        ...
        return cros_build_lib.run(
            ["ssh", ip] + cmd, log_output=True, stdout=stdout
        )

    def _CheckPayloads(self, payload_name):
        """Runs the curl command that checks if payloads have been staged."""
        payload_url = self._GetStagedUrl(
            staged_filename=payload_name, build_id=self._payload_dir
        )
    ...
    def CheckPayloads(self):
        ...
        logging.debug(
            "Checking if payloads have been staged on server %s...",
    ...
    def _GetCurlCmdForPayloadDownload(
        ...
        cmd = [
            "curl",
            "-o",
            os.path.join(payload_dir, saved_filename),
    ...
    def _TransferStatefulUpdate(self):
        ...
        self._device.run(
            self._GetCurlCmdForPayloadDownload(
                payload_dir=self._device.work_dir,
    ...
    def _TransferRootfsUpdate(self):
        ...
        self._device.run(
            self._GetCurlCmdForPayloadDownload(
                payload_dir=self._device_payload_dir,
                build_id=self._payload_dir,
                payload_filename=GetPayloadPropertiesFileName(
                    self._payload_name
                ),
            )
        )
