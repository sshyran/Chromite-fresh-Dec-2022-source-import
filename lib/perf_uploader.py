...
def OutputPerfValue(
    ...
    def ValidateString(param_name, value, max_len):
        if len(value) > max_len:
            raise ValueError(
                "%s must be at most %d characters." % (param_name, max_len)
            )
        ...
        if not set(value).issubset(set(allowed_chars)):
            raise ValueError(
                ...
                "underscores. Its current value is %s." % (param_name, value)
            )
...
PresentationInfo = collections.namedtuple(
    "PresentationInfo", "main_name test_name"
)
...
def _RetryIfServerError(perf_exc):
    ...
    if isinstance(perf_exc, PerfUploadingError):
        return (
            ...
            and perf_exc.orig_exc.code >= 500
        )
    else:
        return False
...
def UploadPerfValues(
    ...
    try:
        ...
        formatted_data = _FormatForUpload(
            perf_data,
            platform_name,
            presentation_info,
            revision=revision,
        ...
        if dry_run:
            logging.debug(
                "UploadPerfValues: skipping upload due to dry-run\n%s",
                formatted_data,
            )
        else:
        ...
        logging.exception(
            "Error when uploading perf data to the perf "
            "dashboard for test %s.",
            test_name,
        )
        raise
    else:
        logging.info(
            "Successfully uploaded perf data to the perf "
            "dashboard for test %s.",
            test_name,
        )
