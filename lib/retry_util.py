...
CURL_STATUS_RE = re.compile(rb"The requested URL returned error: (\d+) ")
...
def _CreateExceptionRetryHandler(exception):
    ...
    if not (
        ...
        or (
            ...
            and all(issubclass(e, Exception) for e in exception)
        )
    ):
        raise TypeError(
            "exceptions should be an exception (or tuple), not %r" % exception
        )
...
class _RetryDelayStrategy(object):
    ...
    def __init__(self, sleep=0, backoff_factor=1, jitter=0):
        ...
        if backoff_factor < 1:
            raise ValueError(
                "backoff_factor must be 1 or greater: %s" % backoff_factor
            )
    ...
    def Sleep(self, attempt):
        ...
        jitter = random.uniform(0.5 * self._jitter, 1.5 * self._jitter)
        total = sleep_duration + jitter
        if total:
            logging.debug(
                "Retrying in %f (%f + jitter %f) seconds ...",
                total,
                sleep_duration,
                jitter,
            )
...
class WithRetry(object):
    ...
    def __init__(
        ...
        if max_retry < 0:
            raise ValueError(
                "max_retry needs to be zero or more: %d" % max_retry
            )
        self._max_retry = max_retry

        if handler is not None and exception is not None:
            raise ValueError(
                "handler and exception cannot be specified at once"
            )
        self._handler = handler or _CreateExceptionRetryHandler(
    ...
    def __call__(self, func):
        ...
        def _Wrapper(*args, **kwargs):
            fname = getattr(
                func, "__qualname__", getattr(func, "__name__", "<nameless>")
            )
            exc_info = None
            for attempt in range(self._max_retry + 1):
                ...
                if attempt and self._log_all_retries:
                    logging.debug(
                        "Retrying %s (attempt %d)", fname, attempt + 1
                    )
                ...
                try:
                    ...
                    if not self._handler(e):
                        logging.debug(
                            "ending retries with error: %s(%s)", e.__class__, e
                        )
                        break
...
def RetryCommand(functor, max_retry, *args, **kwargs):
    ...
    def ShouldRetry(exc):
        ...
        if values is None and exc.returncode < 0:
            logging.info(
                "Child process received signal %d; not retrying.",
                -exc.returncode,
            )
            return False
...
def RunCurl(curl_args, *args, **kwargs):
    ...
    try:
        ...
        raise DownloadError(
            f"Curl failed w/ exit code {e.returncode}: {e.stderr}"
        )
