...
def MountChroot(
    chroot=None, buildroot=None, create=True, proc_mounts="/proc/mounts"
):
    ...
    chroot_vg, chroot_lv = FindChrootMountSource(
    ...
    if chroot_mounts:
        logging.error(
            "Found %s mounted on %s.  Not mounting a chroot over the top",
            ",".join(chroot_mounts),
            chroot,
        )
        return False
    ...
    if not chroot_dev:
        ...
        if chroot_dev:
            logging.debug(
                "Used existing device %s for %s", chroot_dev, chroot_image
            )
        else:
            chroot_dev = _AttachDeviceToFile(chroot_image)
    ...
    if result.returncode != 0:
        cmd = [
            ...
            "-L499G",
            "-T",
            "%s/%s" % (chroot_vg, CHROOT_THINPOOL_NAME),
            "-V500G",
            "-n",
            CHROOT_LV_NAME,
        ]
    ...
    while not os.path.exists(chroot_dev_path):
        ...
        count += 1
        logging.warning(
            "Device file %s does not exist yet on try %d.",
            chroot_dev_path,
            count,
        )
        time.sleep(1)
...
def CleanupChrootMount(
    chroot=None, buildroot=None, delete=False, proc_mounts="/proc/mounts"
):
