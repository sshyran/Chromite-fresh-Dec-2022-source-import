...
class CrosTransferBaseClassTest(cros_test_lib.MockTestCase):
    ...
    def CreateInstance(self, device, **kwargs):
        ...
        return auto_updater_transfer.LocalTransfer(
    ...
    def testErrorTriggerRetryTransferUpdateUtils(self):
        ...
        with remote_access.ChromiumOSDeviceHandler(
            ...
            _MAX_RETRY = self.PatchObject(
                auto_updater_transfer, "_MAX_RETRY", 1
            )
    ...
    def testErrorTriggerRetryTransferStateful(self):
        ...
        with remote_access.ChromiumOSDeviceHandler(
            ...
            self.PatchObject(auto_updater_transfer, "_DELAY_SEC_FOR_RETRY", 1)
            _MAX_RETRY = self.PatchObject(
                auto_updater_transfer, "_MAX_RETRY", 2
            )
            ...
            self.assertRaises(
                cros_build_lib.RunCommandError, transfer.TransferStatefulUpdate
            )
    ...
    def testErrorTriggerRetryTransferRootfs(self):
        ...
        with remote_access.ChromiumOSDeviceHandler(
            ...
            self.PatchObject(auto_updater_transfer, "_DELAY_SEC_FOR_RETRY", 1)
            _MAX_RETRY = self.PatchObject(
                auto_updater_transfer, "_MAX_RETRY", 3
            )
            ...
            self.assertRaises(
                cros_build_lib.RunCommandError, transfer.TransferRootfsUpdate
            )
...
class CrosLocalTransferTest(cros_test_lib.MockTempDirTestCase):
    ...
    def testTransferStatefulUpdateNeedsTransfer(self):
        ...
        self.PatchObject(
            auto_updater_transfer, "STATEFUL_FILENAME", "test_stateful.tgz"
        )
        with remote_access.ChromiumOSDeviceHandler(
            remote_access.TEST_IP
        ) as device:
...
class CrosLabEndToEndPayloadTransferTest(cros_test_lib.MockTempDirTestCase):
    ...
    def CreateInstance(self, device, **kwargs):
        ...
        default_args.update(kwargs)
        return auto_updater_transfer.LabEndToEndPayloadTransfer(
            device=device, **default_args
        )
    ...
    def testTransferUpdateUtilsCurlCalls(self):
        ...
        with remote_access.ChromiumOSDeviceHandler(
            ...
            expected = [
                {
                    "payload_dir": os.path.join(device.work_dir, "src"),
    ...
    def testErrorTransferStatefulServerError(self):
        ...
        with remote_access.ChromiumOSDeviceHandler(
            ...
            self.assertRaises(
                cros_build_lib.RunCommandError, transfer._TransferStatefulUpdate
            )
    ...
    def testErrorTransferRootfsServerError(self):
        ...
        with remote_access.ChromiumOSDeviceHandler(
            ...
            transfer = self.CreateInstance(
                device,
                staging_server="http://wrong:server",
                device_payload_dir="/test/device/payload/dir",
            ...
            self.assertRaises(
                cros_build_lib.RunCommandError, transfer._TransferRootfsUpdate
            )
        ...
        with remote_access.ChromiumOSDeviceHandler(
            ...
            expected = [
                [
                    "curl",
                    "-o",
                    "/test/device/payload/dir/test_update.gz",
                    "http://0.0.0.0:8000/static/test_update.gz",
                ],
                [
                    "curl",
                    "-o",
                    "/test/device/payload/dir/test_update.gz.json",
        ...
        with remote_access.ChromiumOSDeviceHandler(
            ...
            expected = [
                [
                    "curl",
                    "-I",
                    "http://0.0.0.0:8000/static/board-release/12345.6.7/"
                    "test_update.gz",
                    "--fail",
                ],
                [
        ...
        with remote_access.ChromiumOSDeviceHandler(
            ...
            expected = [
                [
                    "curl",
                    "-I",
                    "http://0.0.0.0:8000/static/board-release/"
                    "12345.6.7/test_update.gz",
                    "--fail",
                ],
                [
                    "curl",
    ...
    def testGetPayloadUrlNoImageName(self):
        ...
        with remote_access.ChromiumOSDeviceHandler(
            ...
            url = transfer._GetStagedUrl(staged_filename="payload_filename.ext")
            self.assertEqual(url, expected_url)
