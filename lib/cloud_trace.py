...
def LogSpan(span):
    ...
    # Catch various configuration errors.
        ...
        if error.errno == errno.EPERM:
            ...
            logging.warning(
                "/var/log/traces does not exist; skipping trace log."
            )
            return None
        else:
            raise
...
class Span(collections.abc.Mapping):
    ...
    def __init__(
        self, name, spanId=None, labels=None, parentSpanId=None, traceId=None
    ):
    ...
    def keys(self):
        return (
            key for key in self.__keys if getattr(self, key, None) is not None
        )
...
class SpanStack(object):
    ...
    def __init__(
        ...
        global_context = global_context or os.environ.get(
            self.CLOUD_TRACE_CONTEXT_ENV, ""
        )
