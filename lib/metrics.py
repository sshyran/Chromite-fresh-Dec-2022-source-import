...
MetricCall = collections.namedtuple(
    "MetricCall",
    [
        "metric_name",
        "metric_args",
...
def _FlushingProcessClosed():
    ...
    return (
        FLUSHING_PROCESS is not None and FLUSHING_PROCESS.exitcode is not None
    )
...
class FieldSpecAdapter(object):
    ...
    def __getattr__(self, prop):
        ...
        def func(*args, **kwargs):
            ...
            self._kwargs["field_spec"] = FieldSpecAdapter._InferFieldSpec(
                fields
            )
    ...
    def _InferFieldSpec(fields):
        ...
        return [
            FieldSpecAdapter.FIELD_CLASSES[type(v)](field)
...
@_Metric
def CumulativeMetric(
    name, reset_after=False, description=None, field_spec=_MISSING
):
    """Returns a metric handle for a cumulative float named |name|."""
    return ts_mon.CumulativeMetric(
        name, description=description, field_spec=field_spec
    )
...
@_Metric
def CumulativeSmallIntegerDistribution(
    name, reset_after=False, description=None, field_spec=_MISSING
):
    """Returns a metric handle for a cumulative distribution named |name|.
...
def CumulativeSecondsDistribution(
    name, scale=1, reset_after=False, description=None, field_spec=_MISSING
):
    """Returns a metric handle for a cumulative distribution named |name|.
...
def SecondsInstanceTimerDecorator(
    name,
    fields=None,
    description=None,
    field_spec=_MISSING,
...
class RuntimeBreakdownTimer(object):
    ...
    def __exit__(self, _type, _value, _traceback):
        ...
        unaccounted_metric.add(
            self._GetUnaccountedBreakdown(), fields=self._fields
        )
        ...
        bucketing_loss_metric.increment_by(
            self._GetBucketingLoss(), fields=self._fields
        )
    ...
    def Step(self, step_name):
        ...
        if self._inside_step:
            logging.error(
                "RuntimeBreakdownTimer.Step is not reentrant. "
                "Dropping step: %s",
                step_name,
            )
            yield
    ...
    def _GetStepBreakdowns(self):
        ...
        if not self._total_time_s:
            return {}
        return {
            x.name: (x.time_s * 100) / self._total_time_s
