...
class ChromiumOSUpdater(BaseUpdater):
    ...
    def is_au_endtoendtest(self):
        return isinstance(
            self._transfer_obj, auto_updater_transfer.LabEndToEndPayloadTransfer
        )
    ...
    def CheckRestoreStateful(self):
        ...
        try:
            ...
            if self._do_rootfs_update:
                msg = (
                    "Cannot start nebraska! The stateful partition may be "
                    "corrupted: %s" % e
                )
        ...
        logging.debug(
            "Restore stateful partition is%s required.",
    ...
    def _CheckNebraskaCanRun(self):
        ...
        nebraska_bin = os.path.join(
            self.device_dev_dir, self.REMOTE_NEBRASKA_FILENAME
        )
    ...
    def GetUpdateStatus(cls, device, keys=None):
        ...
        result = device.run(
            [cls.REMOTE_UPDATE_ENGINE_BIN_FILENAME, "--status"],
            capture_output=True,
            log_output=True,
        )
        ...
        for key in keys:
            if key not in status:
                raise ValueError(
                    'Missing "%s" in the update engine status' % key
                )
    ...
    def _StartUpdateEngineIfNotRunning(self, device):
        ...
        try:
            result = device.run(
                ["start", "update-engine"], capture_output=True, log_output=True
            ).stdout
    ...
    def UpdateRootfs(self):
        ...
        try:
            ...
            while True:
                ...
                logging.info(
                    "Waiting for update...status: %s at progress %s",
                    op,
                    progress,
                )
                ...
                if oper is not None:
                    if op == UPDATE_STATUS_DOWNLOADING:
                        ...
                        end_message_not_printed
                        and op == UPDATE_STATUS_FINALIZING
                    ):
                        oper.Cleanup()
        ...
        # need to revert the boot partition if the Nebraska fails to start, etc.
            ...
            logging.warning(
                nebraska.PrintLog() or "No nebraska log is available."
            )
    ...
    def _ResolveAPPIDMismatchIfAny(self):
        ...
        if self.device.app_id != payload_app_id:
            logging.warning(
                "You are installing an image with a different release "
    ...
    def _Reboot(self, error_stage, timeout=None):
        try:
            ...
            raise ChromiumOSUpdateError(
                "Could not recover from reboot at %s" % error_stage
            )
    ...
    def _cgpt(self, flag, kernel, dev="$(rootdev -s -d)"):
        ...
        return int(
            self._RetryCommand(
                cmd, capture_output=True, log_output=True
            ).stdout.strip()
        )
    ...
    def _GetReleaseVersion(self):
        ...
        lsb_release_content = self._RetryCommand(
            ["cat", "/etc/lsb-release"], capture_output=True, log_output=True
        ).stdout.strip()
    ...
    def _VerifyBootExpectations(self, expected_kernel_state, rollback_message):
        ...
        if (
            ...
            and active_kernel_state != expected_kernel_state
        ):
            logging.debug("Dumping partition table.")
            self.device.run(
                ["cgpt", "show", "$(rootdev -s -d)"], **self._cmd_kwargs
            )
        ...
        try:
            timeout_util.WaitForReturnTrue(
                lambda: (
                    self._GetKernelTries(active_kernel_state) == 0
    ...
    def _CheckVersionToConfirmInstall(self):
        logging.debug(
            "Checking whether the new build is successfully installed..."
        )
        if not self.update_version:
            logging.debug(
                "No update_version is provided if test is executed with"
                "local nebraska."
            )
            return True
            ...
            for folder in self.REMOTE_STATEFUL_PATH_TO_CHECK:
                ...
                if self.device.IfFileExists(
                    ...
                    raise StatefulUpdateError(
                        "failed to post-check stateful update."
                    )
    ...
    def _IsUpdateUtilsPackageInstalled(self):
        ...
        try:
            ...
            if not self.device.IfFileExists(
                ...
                logging.info(
                    "Nebraska files not found on device. Resending them..."
                )
            ...
            logging.warning(
                "Failed to verify whether packages still exist: %s", e
            )
            return False
    ...
    def PostCheckRootfsUpdate(self):
        ...
        logging.debug(
            "active_kernel= %s, inactive_kernel=%s",
            active_kernel,
            inactive_kernel,
        )
        if self._GetKernelPriority(inactive_kernel) < self._GetKernelPriority(
            active_kernel
        ):
            raise RootfsUpdateError(
                "Update failed. The priority of the inactive "
        ...
        while True:
            try:
                ...
                if start_verify_time - start_time < POST_CHECK_SETTLE_SECONDS:
                    logging.warning(
                        "Delaying for re-check of %s to update to %s (%s)",
                        self.device.hostname,
                        self.update_version,
                        e,
                    )
