...
TEST_CASES_PARSE_OR = (
    ...
    ('BSD-1 || ( BSD GPL ) || ( BSD-2 GPL-3 )', None,
     ['BSD-1', 'BSD', 'BSD-2']),
)

@pytest.mark.parametrize('test,exp,reduced', TEST_CASES_PARSE_OR)
def test_parse_or(test, exp, reduced):
  """Verify ||() parsing."""
  if exp is None:
    exp = test
  licenses = ebuild_license.parse(test)
...
def test_or_reduce():
  ...
  assert licenses.reduce(or_reduce=pick1) == ['GPL-1', 'BSD-1']

  def pick2(choices):
    return 'BSD-2' if 'BSD-2' in choices else choices[0]
  assert licenses.reduce(or_reduce=pick2) == ['GPL-1', 'BSD-1']
  assert licenses.reduce(
      use_flags=['foo'], or_reduce=pick2) == ['GPL-1', 'BSD-2']

  def pick3(choices):
    return 'BSD-3' if 'BSD-3' in choices else choices[0]
  assert licenses.reduce(or_reduce=pick3) == ['GPL-1', 'BSD-3']
