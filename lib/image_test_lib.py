...
class ImageTestCase(unittest.TestCase, _BoardAndDirectoryMixin):
    ...
    def OutputPerfValue(
        self, description, value, units, higher_is_better=True, graph=None
    ):
        """Record a perf value.
        ...
        if not self._result_dir:
            logging.warning(
                "Result directory is not set. Ignore OutputPerfValue."
            )
            return
        if graph is None:
        ...
        perf_uploader.OutputPerfValue(
            file_name, description, value, units, higher_is_better, graph
        )
