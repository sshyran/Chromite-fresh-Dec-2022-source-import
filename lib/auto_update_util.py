...
def VersionMatch(build_version, release_version):
    ...
    release_version_no_date = re.sub(
        r"\d{4}_\d{2}_\d{2}_\d+", "0", release_version
    )
    ...
    if is_trybot_non_release_build:
        if not has_date_string:
            logging.error(
                "A trybot paladin or pre-cq build is expected. "
        ...
        if not has_pgo_generate:
            logging.error(
                "A pgo-generate build is expected. Version "
        ...
        if has_date_string:
            logging.error(
                "Unexpected date found in a non trybot paladin or "
                "pre-cq build."
            )
            return False
