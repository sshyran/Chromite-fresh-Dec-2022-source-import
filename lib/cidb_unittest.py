...
class HelperFunctionsTest(cros_test_lib.TestCase):
    ...
    def testIsRetryableExceptionMatch(self):
        ...
        self.assertTrue(
            cidb._IsRetryableException(
                self._WrapError(RetryableOperationalError())
            )
        )
        self.assertFalse(
            cidb._IsRetryableException(self._WrapError(FatalOperationalError()))
        )
        self.assertFalse(
            cidb._IsRetryableException(self._WrapError(UnknownError()))
        )
...
class CIDBConnectionFactoryTest(cros_test_lib.MockTestCase):
    ...
    def testSetupMock(self):
        ...
        self.assertTrue(cidb.CIDBConnectionFactory.IsCIDBSetup())
        self.assertEqual(
            cidb.CIDBConnectionFactory.GetCIDBConnectionForBuilder(), a
        )
        ...
        self.assertEqual(
            cidb.CIDBConnectionFactory.GetCIDBConnectionForBuilder(), b
        )
    ...
    def testSetupNo(self):
        ...
        self.assertTrue(cidb.CIDBConnectionFactory.IsCIDBSetup())
        self.assertEqual(
            cidb.CIDBConnectionFactory.GetCIDBConnectionForBuilder(), None
        )
        self.assertRaises(
            factory.ObjectFactoryIllegalOperation,
            cidb.CIDBConnectionFactory.SetupProdCidb,
...
class SchemaVersionedMySQLConnectionTest(cros_test_lib.MockTempDirTestCase):
    ...
    def setUp(self):
        ...
        self.PatchObject(
            cidb.SchemaVersionedMySQLConnection,
            "_ExecuteWithEngine",
    ...
    def testConnectionWithUnixSocket(self):
        ...
        conn = cidb.SchemaVersionedMySQLConnection(
            self.db_name,
            cidb.CIDB_MIGRATIONS_DIR,
            self.tempdir,
            for_service=True,
        )
        self.assertEqual(
            str(conn._connect_url), "mysql+pymysql://user:password@/cidb"
        )

        unix_socket_path = os.path.join(self.tempdir, "unix_socket.txt")
        osutils.WriteFile(
            unix_socket_path, "/cloudsql/CLOUDSQL_CONNECTION_NAME"
        )
