...
class TestImportKeyset(cros_test_lib.MockTempDirTestCase):
    ...
    def test_ImportKeys(self):
        ...
        for sname, dname, settype in data:
            ...
            expected_keysets.setdefault("%s-keysets" % settype, []).append(
                sname
            )
    ...
    def test_ImportKeysChecksExists(self):
        ...
        kd = self.keydata
        osutils.WriteFile(
            os.path.join(kd.configs, "foo-mp.yaml"), "bad: 0", makedirs=True
        )
        self.assertRaises(
            update_release_keys.KeyimportError,
            kd.ImportKeyset,
            "foo-mp",
            "FooMPKeys",
        )
...
class TestRepoYaml(cros_test_lib.MockTempDirTestCase):
    ...
    def test_ReadAssertsProperly(self):
        ...
        osutils.WriteFile(
            os.path.join(self.base, "contents.yaml"), "metadata-version: 9\n"
        )
        options = update_release_keys.ParseArgs(self.argv)
        self.assertRaises(
            update_release_keys.KeyimportError,
    ...
    def test_WriteRepoYaml(self):
        ...
        self.assertEqual(
            kd.contents_yaml, os.path.join(self.base, "contents.yaml")
        )
        kd.keysets = {"metadata-version": 0, "mp-keysets": ["foo-mp"]}
        kd.WriteRepoYaml()
        self.assertEqual(
            osutils.ReadFile(kd.contents_yaml),
            "metadata-version: 0\nmp-keysets:\n- foo-mp\n",
        )
